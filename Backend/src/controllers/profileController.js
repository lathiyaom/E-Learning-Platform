    ...
    try {
      if (req.user.role === "tenant") {
        return res.status(200).json({
          ...
          data: { emailNotifications: true, pushNotifications: true, smsNotifications: false, newsletter: false, twoFactorAuth: false, privacy: "public" },
        });
      }
      const userId = req.user.id;
      const user = await User.findById(userId);
  ...
  updateSettings: async (req, res) => {
    try {
      if (req.user.role === "tenant") {
        return res.status(400).json({
          success: false,
          message: "Organization account settings are managed from organization settings",
        });
      }
      const userId = req.user.id;
